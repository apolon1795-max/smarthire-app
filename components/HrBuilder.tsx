...
const HrBuilder: React.FC<HrBuilderProps> = ({ scriptUrl, company, onExit, onTestPreview }) => {
  ...
  const loadJobs = async () => {
    ...
    try {
      ...
      // Если сервер вернул ошибку вместо массива (напр. объект {status: "error"}), ставим пустой массив
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (data.status === 'success') {
        ...
        // После сохранения сразу пробуем обновить локальный список
        await loadJobs();
      } else {
        alert("Ошибка сервера: " + (data.message || "неизвестно"));
      }
    } catch (e) { 
