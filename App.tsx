...
export default function App() {
  ...
  useEffect(() => {
    try {
      ...
      setInitError("Ошибка при инициализации приложения. Попробуйте очистить кеш.");
    }
  }, []);
  ...
  const fetchCustomConfig = async (jobId: string) => {
    if (SCRIPT_URL.includes('ВАШ_УНИКАЛЬНЫЙ_ID')) {
      console.warn("SCRIPT_URL не настроен");
      return;
    }
  ...
  const calculateHexacoScores = (answers: UserAnswers): HexacoScore[] => {
    ...
    return Object.entries(scores).map(([code, data]) => {
      ...
      return { factor: FACTOR_NAMES[code], code: code, rawScore: data.sum, questionCount: data.count, average: average, percentage: ((average - 1) / 4) * 100 };
    });
  };
  ...
  const handleSectionComplete = (sectionId: string, answers: UserAnswers) => {
    ...
    if (sectionId === 'conscientiousness') {
      ...
      validityProfile = { attentionPassed: attentionVal, lieScore: (Number(answers['check_lie_1'] || 1) + Number(answers['check_lie_2'] || 1)) / 2, statusLabel: attentionVal ? 'Valid' : 'FAIL' };
    ...
    testSections.forEach(section => {
      ...
      section.questions.forEach(q => {
        if (q.type === 'likert') {
          ...
          answers[q.id] = q.options![Math.floor(Math.random() * q.options!.length)].value;
        } else if (q.type === 'text') {
          answers[q.id] = "Тестовый ответ кандидата.";
        }
      });
      allResults.push({ sectionId: section.id as any, title: section.title, rawScore: 3, maxScore: 5, percentage: 60, answers });
  ...
  const handleLoginSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (COMPANY_CODES[code]) {
      ...
      setIsAuthenticated(true);
      if (!customJobId) { setShowHrBuilder(true); setIsHrMode(true); }
    } else {
      alert("Неверный код доступа.");
    }
  };

  if (initError) return <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center text-white p-10"><AlertTriangle className="text-red-500 mb-4" size={48}/><p>{initError}</p><button onClick={() => resetApp(true)} className="mt-4 bg-blue-600 px-6 py-2 rounded">Сбросить всё</button></div>;

  if (isLoadingConfig) return <div className="min-h-screen bg-slate-950 flex items-center justify-center text-blue-500 font-bold animate-pulse text-center p-10"><div>Синхронизация с облаком...<br/><span className="text-xs text-slate-500">Загрузка конфигурации вакансии</span></div></div>;

  const ControlBar = () => (
    <div className="fixed top-4 right-4 z-[9999] flex items-center gap-2">
      {currentCompany && <div className="bg-blue-600/20 border border-blue-500/30 px-3 py-1 rounded-lg text-[10px] font-black text-blue-400 uppercase tracking-widest hidden sm:block">Company: {currentCompany}</div>}
      {isHrMode && candidateInfo && <button onClick={handleAutofillAll} className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-500 text-white px-4 py-2 rounded-xl text-xs font-bold transition-all shadow-xl shadow-indigo-900/40"><Wand2 size={14}/> МАГИЯ (АВТО)</button>}
      {(candidateInfo || isAuthenticated || customJobId || showHrBuilder) && <button onClick={() => resetApp(true)} className="flex items-center gap-2 bg-slate-900/80 backdrop-blur-md border border-slate-700 text-slate-300 hover:text-white px-4 py-2 rounded-xl text-xs font-bold transition-all shadow-lg hover:bg-red-900/40 hover:border-red-500/50"><LogOut size={14}/> ВЫЙТИ</button>}
    </div>
  );
      ...
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-6 text-center">
        <div className="max-w-xl w-full">
           <div className="mb-12 space-y-4">
              <div className="inline-flex p-4 rounded-3xl bg-blue-600/10 border border-blue-500/20 mb-4"><ShieldCheck className="text-blue-500" size={64} /></div>
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-50 font-sans">
      ...
      <header className="max-w-7xl mx-auto py-12 px-4 sm:px-6 text-center relative">
        ...
        <button onClick={() => setShowHrBuilder(true)} className="mt-8 inline-flex items-center gap-2 bg-slate-900 border border-slate-800 px-6 py-3 rounded-2xl text-xs font-bold text-slate-400 hover:text-white hover:border-blue-500/50 transition-all"><Settings size={16} /> ПЕРЕЙТИ В HR-КАБИНЕТ</button>
      </header>
